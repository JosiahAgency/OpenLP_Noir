...
def insert_soft_splits(text, threshold):
    """
    Splits long paragraphs cleanly at sentence boundaries using the OpenLP soft-split
    tag [---]. It checks character lengths iteratively, accumulating sentences up to
    the threshold before placing a tag, preventing middle-of-sentence breaks.
    """
    if len(text) <= threshold:
        return text

    # Split on sentence terminals (. ! ?) followed by a space (preserves formatting)
    ...
    for sentence in sentences:
        ...
        # If adding the next sentence exceeds the threshold, save current chunk and start a new one
        if current_length + sentence_len > threshold and current_chunk:
...
def convert(pdf_path, expect_gap_at_headings=True, split_threshold=None):
    """
    ...
    :param split_threshold: Optional character limit above which paragraphs are softly split with [---].
    ...
    def flush_paragraph():
        ...
        if chapter is not None and paragraph_lines:
            ...
            if text:
                if split_threshold:
                    text = insert_soft_splits(text, split_threshold)
...
def main():
    ...
    parser.add_argument('--split-threshold', type=int, default=300,
                        help='Character limit threshold for soft-splitting long paragraphs with [---] (default: 300, set to 0 to disable)')
    ...
    # Configure split threshold
    threshold = args.split_threshold if args.split_threshold > 0 else None
    if threshold:
        print('Soft-split active: Paragraphs exceeding {t} characters will split with "[---]"'.format(t=threshold))
    else:
        print('Soft-split inactive.')

    chapters, warnings = convert(args.pdf, split_threshold=threshold)
    if not chapters:
