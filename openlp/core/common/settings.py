...
from openlp.core.common.enum import (
    AlertLocation,
    BibleSearch,
    CustomSearch,
    EGWSearch,
...
def upgrade_screens(
        number, x_position, y_position, height, width, can_override, is_display_screen
):
    """
...
if is_win():
    ...
    def _wingreg_subkeys(path):
        ...
        """
        with (
            suppress(WindowsError),
            winreg.OpenKey(winreg.HKEY_CURRENT_USER, path, 0, winreg.KEY_READ) as k,
        ):
    ...
    if settings_version_tuple < (3, 1, 99):
        ...
        if is_linux() or settings.format() == Settings.IniFormat:
            ...
            with open(settings_filename, "r+") as settings_file:
                ...
                settings_file.write(
                    file_contents.replace("xePyQt_PyObject", "tPyObject")
                )
                    ...
                    if value[2] == winreg.REG_BINARY:
                        ...
                        if "@Variant" in decoded_value:
                            ...
                            with winreg.OpenKey(
                                ...
                                winreg.SetValueEx(
                                    k, value[0], 0, winreg.REG_BINARY, migrated
                                )
            # reload migrated settings
            settings = Settings()
            return settings
            ...
            for key, value in plistdata.items():
                if isinstance(value, (bytes, bytearray)):
                    if value.startswith(b"@Variant"):
                        migrated_value = value.replace(
                            b"\x0ePyQt_PyObject", b"\tPyObject"
                        )
...
class Settings(QtCore.QSettings):
    ...
    * Adds functionality for OpenLP Portable. If the ``defaultFormat`` is set to ``IniFormat``, and the path to the Ini
    ...
    __default_settings__ = {
        ...
        "alerts/font face": "Lato",
        "alerts/font size": 40,
        # JSON object of per-priority style preset overrides; empty means the
        # built-in defaults in openlp.plugins.alerts.lib.presets
        "alerts/presets": "",
        ...
        "bibles/footer show reference": True,
        "bibles/footer show version": True,
        "bibles/footer show copyright": True,
        "bibles/footer show permission": True,
        "bibles/reference placement": ReferencePlacement.Footer,
        ...
        "egwlibrary/reference placement": ReferencePlacement.Footer,
        ...
        "SettingsImport/file_date_created": datetime.datetime.now().strftime(
            "%Y-%m-%d %H:%M"
        ),
    ...
    __setting_upgrade_1__ = [
        ...
        (
            "media/players",
            "media/players_temp",
            [(media_players_conv, None)],
        ),  # Convert phonon to system
        (
            "media/players_temp",
            "media/players",
            [],
    ...
    __setting_upgrade_2__ = [
        ...
        (
            "advanced/default color",
            "core/logo background color",
            [],
        ),  # Default image renamed + moved to general > 2.4.
        (
            "advanced/default image",
            "core/logo file",
            [],
        ),  # Default image renamed + moved to general after 2.4.
        (
            "advanced/use_dark_style",
            "advanced/ui_theme_name",
            [(upgrade_dark_theme_to_ui_theme, [False])],
        ),
        (
            "bibles/advanced bible",
            "",
            [],
        ),  # Common bible search widgets combined in 2.6
        (
        ...
        (
            ...
            "",
            [],
        ),  # Just remove these bible proxy settings. They weren't used in 2.4!
        ...
        (
            ...
            "core/screens",
            [(upgrade_screens, [1, 0, 0, None, None, False, False])],
        ),
        ...
        (
            ...
            "shortcuts/userManualItem",
            [],
        ),  # Online and Offline help were combined in 2.6.
        (
    ...
    __setting_upgrade_3__ = [
        (
            "songs/add songbook slide",
            "songs/first slide mode",
            [(upgrade_add_first_songbook_slide_config, False)],
        )
    ]
    ...
    def set_up_default_values():
        ...
        Settings.__default_settings__["advanced/default service name"] = (
    ...
    def __init__(self, *args):
        ...
        if (
                not args
                and Settings.__file_path__
                and Settings.defaultFormat() == Settings.IniFormat
        ):
    ...
    def init_default_shortcuts(self):
        ...
        Settings.__default_settings__.update(
            {
                "shortcuts/aboutItem": [
                    QtGui.QKeySequence(QtCore.Qt.Key.Key_Control + QtCore.Qt.Key.Key_F1)
                ],
                ...
                "shortcuts/desktopScreen": [
                    QtGui.QKeySequence(QtCore.Qt.Key.Key_D),
                ...
                "shortcuts/listViewTimerDeleteItem": [
                    QtGui.QKeySequence(QtGui.QKeySequence.StandardKey.Delete)
                ],
                "shortcuts/listViewTimerServiceItem": [
                    QtGui.QKeySequence(QtCore.Qt.Key.Key_Plus),
                    QtGui.QKeySequence(QtCore.Qt.Key.Key_Equal),
                ],
                "shortcuts/lockPanel": [],
                ...
                "shortcuts/viewProjectorManagerItem": [
                    QtGui.QKeySequence(QtCore.Qt.Key.Key_F6)
                ],
                "shortcuts/viewThemeManagerItem": [
                    QtGui.QKeySequence(QtCore.Qt.Key.Key_F10)
                ],
                "shortcuts/viewMediaManagerItem": [
                    QtGui.QKeySequence(QtCore.Qt.Key.Key_F8)
                ],
                "shortcuts/viewPreviewPanel": [
                    QtGui.QKeySequence(QtCore.Qt.Key.Key_F11)
                ],
                "shortcuts/viewLivePanel": [QtGui.QKeySequence(QtCore.Qt.Key.Key_F12)],
                "shortcuts/viewServiceManagerItem": [
                    QtGui.QKeySequence(QtCore.Qt.Key.Key_F9)
                ],
    ...
    def upgrade_settings(self):
        ...
        for version in range(current_version, __version__):
            version += 1
            upgrade_list = getattr(
                self, "__setting_upgrade_{version}__".format(version=version)
            )
            for old_keys, new_key, rules in upgrade_list:
                ...
                if any([not self.contains(old_key) for old_key in old_keys]):
                    log.warning(
                        "One of {} does not exist, skipping upgrade".format(old_keys)
                    )
                    continue
                if new_key:
                    # Get the value of the old_key.
                    old_values = [
                        super(Settings, self).value(old_key) for old_key in old_keys
                    ]
                    ...
                    if rules:
                        ...
                        old_values = [
                            self._convert_value(old_value, default_value)
                            for old_value, default_value in zip(
                                old_values, default_values
                            )
                        ]
    ...
    def setValue(self, key, value):
        ...
        if (
                ...
                or (isinstance(value, list) and value and is_serializable(value[0]))
        ):
    ...
    def _convert_value(self, setting, default_value):
        ...
        **Note**, this method only converts a few types and might need to be extended if a certain type is missing!
        ...
        if setting is None:
            ...
            if (
                    "json_meta" in setting
                    or "__Path__" in setting
                    or setting.startswith("{")
            ):
    ...
    def export(self, dest_path):
        ...
        # Get the settings.
        keys = self.allKeys()
        export_settings = QtCore.QSettings(
            str(temp_path), QtCore.QSettings.Format.IniFormat
        )
        ...
        export_settings.setValue(
            "SettingsImport/file_date_created", now.strftime("%Y-%m-%d %H:%M")
        )
        ...
        try:
            with (
                dest_path.open("w") as export_conf_file,
                temp_path.open("r") as temp_conf,
            ):
